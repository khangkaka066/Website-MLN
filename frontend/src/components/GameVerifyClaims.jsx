...
const submitGameResult = async (score, total, history) => {
  ...
  try {
    ...
    if (!data.duplicate) toast.success("Điểm số đã được lưu vào hệ thống!");
  } catch (error) {
    console.error("Lỗi khi gửi điểm:", error);
  }
};

// ----- Stats Summary (OUTSIDE main component to follow Rules of Hooks) -----
const StatsSummary = ({ stats, loadingStats }) => {
  const chartRef = useRef(null);
  const chartInstance = useRef(null);

  useEffect(() => {
    if (!stats?.claim_stats || !chartRef.current) return;
    ...
    chartInstance.current = new Chart(chartRef.current, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { callbacks: { label: (ctx) => ` ${ctx.parsed.y}% đúng` } },
        },
        scales: {
          y: {
            min: 0,
  ...
  if (loadingStats) {
    return (
      <div className="flex items-center gap-2 p-2">
        <div className="size-3 animate-pulse rounded-full bg-[hsl(174_55%_70%)]" />
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        {[
          { icon: "👥", label: "Người chơi", value: stats.total_players },
          { icon: "⭐", label: "Điểm TB", value: stats.average_score },
          { icon: "✅", label: "Đúng TB", value: `${stats.average_correct_pct}%` },
        ].map((m) => (
      ...
      <div>
        ...
        <div className="mb-1.5 flex gap-3">
          {[
            { color: "#1D9E75", label: "≥75%" },
            { color: "#BA7517", label: "40–74%" },
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-xl bg-[hsl(152_55%_94%)] p-4">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="size-4 text-[hsl(152_60%_22%)]" strokeWidth={2.25} />
          ...
          <div className="rounded-xl bg-[hsl(24_100%_94%)] p-4">
            <div className="flex items-center gap-2">
              <Search className="size-4 text-[hsl(24_94%_30%)]" strokeWidth={2.25} />
...
// ----- Question Screen -----
const QuestionScreen = ({ claim, index, total, score, streak, onChoose, history }) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
      <RevealOnScroll className="lg:col-span-8" key={claim.id}>
        <Card className="rounded-2xl border-border bg-card p-6 shadow-[0_10px_30px_-22px_rgba(15,23,42,0.35)] sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <Badge variant="outline" className="rounded-full border-border bg-card font-mono-doc text-xs uppercase tracking-wider">
              Câu {index + 1} / {total}
            </Badge>
          </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="flex flex-col gap-2">
              ...
              <div data-testid="game-claim-text" className="flex-1 rounded-xl bg-[hsl(48_100%_97%)] p-5 sm:p-6">
          ...
          <p className="mt-5 text-sm text-muted-foreground">
            Theo bạn, câu khẳng định này có thể đưa thẳng vào bài học thuật của bạn không?
          </p>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <span className="flex flex-col">
                ...
                <span className="text-xs font-normal text-[hsl(152_60%_22%)]/80">Tin cậy, có thể dùng trong bài</span>
              </span>
            </Button>
            <Button
            ...
            >
              ...
              <span className="flex flex-col">
                <span className="font-display text-lg font-extrabold">Cần kiểm chứng</span>
                <span className="text-xs font-normal text-[hsl(24_94%_25%)]/80">Có dấu hiệu đáng ngờ, áp dụng 5 bước</span>
              </span>
            </Button>
          </div>
        </Card>
      ...
      <RevealOnScroll className="lg:col-span-4" delay={0.05}>
        <Card data-testid="game-scoreboard" className="sticky top-20 rounded-2xl border-border bg-card p-5 shadow-[0_10px_30px_-22px_rgba(15,23,42,0.35)] sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <p className="font-display text-sm font-bold text-foreground">Bảng điểm</p>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-secondary p-3">
              <p className="text-xs text-muted-foreground">Điểm</p>
              <p className="font-display text-2xl font-extrabold text-foreground" data-testid="game-score">{score}</p>
            </div>
            <div className="rounded-xl bg-secondary p-3">
              <p className="text-xs text-muted-foreground">Streak</p>
              <p className="font-display text-2xl font-extrabold text-foreground" data-testid="game-streak">🔥{streak}</p>
            </div>
          </div>
          <div className="mt-4">
            <Progress value={progress} data-testid="game-progress" className="h-2" />
            <p className="mt-2 text-xs text-muted-foreground">Tiến độ: {Math.round(progress)}%</p>
          </div>
          {history.length > 0 && (
            <div className="mt-5">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Đã trả lời</p>
...
const ResultScreen = ({ score, total, history, onReplay }) => {
  ...
  const verdict =
    pct >= 80
      ? { title: "Bạn là người kiểm chứng cứng cựa!", desc: "Bạn đã nhận ra hầu hết các bẫy của AI. Áp dụng tiếp 5 bước cho các bài thực tế nhé.", color: "hsl(152 60% 22%)", bg: "hsl(152 55% 94%)" }
      : pct >= 50
        ? { title: "Bạn đã có phản xạ nghi vấn!", desc: "Tốt — nhưng vẫn còn vài câu trượt. Xem lại 5 bước để bịt kín lỗ hổng.", color: "hsl(24 94% 30%)", bg: "hsl(24 100% 94%)" }
        : { title: "AI đã đánh lừa được bạn nhiều lần.", desc: "Đó chính xác là lý do bộ thẻ 5 bước tồn tại. Hãy xem 5 bước và chơi lại.", color: "hsl(0 72% 35%)", bg: "hsl(0 90% 96%)" };

  return (
    <RevealOnScroll>
      <Card data-testid="game-result-card" className="rounded-2xl border-border bg-card p-6 shadow-[0_10px_30px_-22px_rgba(15,23,42,0.35)] sm:p-10">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-10">
          <div className="lg:col-span-5">
            ...
            <h3 className="mt-3 font-display text-3xl font-extrabold text-foreground sm:text-4xl">{verdict.title}</h3>
            <p className="mt-3 text-base leading-relaxed text-muted-foreground">{verdict.desc}</p>
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-secondary p-4">
                <p className="text-xs text-muted-foreground">Đúng / Tổng</p>
                <p className="font-display text-3xl font-extrabold text-foreground">
                  {correctCount}<span className="text-base font-semibold text-muted-foreground"> / {total}</span>
                </p>
              </div>
              <div className="rounded-xl p-4" style={{ background: verdict.bg }}>
                <p className="text-xs" style={{ color: verdict.color }}>Tỉ lệ</p>
                <p className="font-display text-3xl font-extrabold" style={{ color: verdict.color }}>{pct}%</p>
              </div>
            </div>
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <Button data-testid="game-replay-button" onClick={onReplay} variant="outline" className="rounded-xl">
                <RotateCcw className="mr-2 size-4" strokeWidth={2} />
                Chơi lại
              </Button>
              <Button data-testid="game-go-steps-button" onClick={() => scrollTo("steps")} className="rounded-xl bg-primary text-primary-foreground hover:bg-[hsl(var(--primary)/0.92)]">
          ...
          <div className="lg:col-span-7">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Chi tiết từng câu</p>
            <ul className="mt-3 space-y-2">
              {history.map((h, i) => (
                <li key={h.id} className={`flex items-start gap-3 rounded-xl p-3 ring-1 ${h.correct ? "bg-[hsl(152_55%_94%)] ring-[hsl(152_60%_35%/0.25)]" : "bg-[hsl(0_90%_97%)] ring-[hsl(0_72%_52%/0.2)]"}`}>
                  <span className={`inline-flex size-7 shrink-0 items-center justify-center rounded-md font-mono-doc text-xs font-bold ${h.correct ? "bg-[hsl(152_55%_88%)] text-[hsl(152_60%_18%)]" : "bg-[hsl(0_90%_92%)] text-[hsl(0_72%_30%)]"}`}>
                    {i + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="line-clamp-2 text-sm font-medium text-foreground">{h.text}</p>
                    <p className="mt-1 flex items-center gap-1.5 text-xs">
                      {h.correct
                        ? <CheckCircle2 className="size-3.5 text-[hsl(152_60%_30%)]" strokeWidth={2.25} />
                        : <XCircle className="size-3.5 text-[hsl(0_72%_42%)]" strokeWidth={2.25} />}
                      <span className="text-muted-foreground">
                        Đáp án đúng: <span className="font-semibold text-foreground">{h.correctLabel}</span>{" "}
                        — Bạn chọn: <span className="font-semibold text-foreground">{h.userLabel}</span>
                      </span>
                    </p>
                  </div>
                </li>
...
const RevealDialog = ({ open, payload, onNext, isLast }) => (
  <Dialog open={open} onOpenChange={() => {}}>
    <DialogContent data-testid="game-reveal-dialog" className="max-w-lg rounded-2xl bg-card" onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
      {payload && (
        <>
          <DialogTitle className="sr-only">{payload.correct ? "Chính xác" : "Chưa đúng"} — Đáp án: {payload.correctLabel}</DialogTitle>
          ...
          <div className="flex items-center gap-2">
            {payload.correct ? (
              <Badge className="rounded-full border-0 bg-[hsl(152_55%_92%)] text-[hsl(152_60%_22%)] hover:bg-[hsl(152_55%_92%)]">
                <CheckCircle2 className="mr-1.5 size-3.5" strokeWidth={2.25} />
                Chính xác
              </Badge>
            ) : (
            ...
            <Badge variant="outline" className="rounded-full border-border bg-card">{payload.tag}</Badge>
          </div>
          <p className="mt-2 font-display text-lg font-extrabold text-foreground">Đáp án đúng: {payload.correctLabel}</p>
          <div className="rounded-xl bg-secondary p-3 text-sm leading-relaxed text-foreground">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Vì sao?</p>
          ...
          <div className="rounded-xl border border-dashed border-border bg-[hsl(48_100%_97%)] p-3 text-sm leading-relaxed text-foreground">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(174_62%_28%)]">Cách xử lý</p>
            <p className="mt-1">{payload.fix}</p>
          </div>
          <Button data-testid="game-next-button" onClick={onNext} className="mt-2 w-full rounded-xl bg-primary text-primary-foreground hover:bg-[hsl(var(--primary)/0.92)]">
...
export const GameVerifyClaims = () => {
  ...
  const handleChoose = (choice) => {
    ...
    setHistory((h) => [...h, { id: claim.id, text: claim.text, correct, userLabel, correctLabel }]);
    setReveal({ correct, correctLabel, tag: claim.tag, why: claim.why, fix: claim.fix });

    if (correct) {
      toast.success(`+${points} điểm — Chính xác!`, {
        description: streak > 0 ? `Streak x${streak + 1} (+${streak * 2} điểm bonus)` : "Tiếp tục giữ phong độ!",
      });
    } else {
      toast("Chưa đúng — mở chi tiết để hiểu vì sao", { icon: <AlertTriangle className="size-4" /> });
    }
  };
  ...
  return (
    <section id="game" data-testid="game-section" className="relative isolate scroll-mt-24 overflow-hidden bg-[hsl(210_40%_98%)] py-16 sm:py-20 lg:py-24">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <RevealOnScroll>
          ...
          <p className="mt-2 max-w-2xl text-base leading-relaxed text-muted-foreground">
            Trò chơi nhanh để rèn "phản xạ nghi vấn" trước mọi output AI. Quét QR để chơi cùng cả lớp, hoặc bấm bắt đầu để chơi ngay tại đây.
          </p>
        </RevealOnScroll>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <div className="flex flex-col gap-4 lg:col-span-5">
                ...
                <Card className="rounded-2xl border-border bg-card p-4 shadow-sm">
                  <StatsSummary stats={stats} loadingStats={loadingStats} />
                </Card>
              </div>
            </div>
          )}
          {phase === "playing" && (
            <QuestionScreen claim={claim} index={index} total={total} score={score} streak={streak} onChoose={handleChoose} history={history} />
          )}
          {phase === "finished" && (
            <ResultScreen score={score} total={total} history={history} onReplay={handleStart} />
          )}
        </div>
      </div>

      <RevealDialog open={!!reveal} payload={reveal} onNext={handleNext} isLast={index + 1 >= total} />
    </section>
  );
};
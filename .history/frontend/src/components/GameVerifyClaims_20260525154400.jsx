...
export const GameVerifyClaims = () => {
  ...
  const handleReplay = () => {
    ...
    useEffect(() => {
      // Chỉ vẽ khi có dữ liệu và thẻ canvas đã tồn tại trong DOM
      ...
      const labels = stats.claim_stats.map((c) => c.claim_id.replace("claim-", "Câu "));
      const dataValues = stats.claim_stats.map((c) => c.correct_pct);
      
      // Màu sắc dựa trên độ khó
      const bgColors = dataValues.map((v) => {
        if (v >= 75) return "#1D9E75"; // Xanh lá
        if (v >= 40) return "#BA7517"; // Cam
        return "#E24B4A";              // Đỏ
      });
      ...
      chartInstance.current = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (context) => ` Tỉ lệ đúng: ${context.parsed.y}%`,
              },
            },
          },
          scales: {
    ...
    if (!stats) {
      return <p className="text-xs text-center text-muted-foreground py-4">Chưa có dữ liệu thống kê.</p>;
    }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl bg-secondary/50 p-2 text-center border border-border/50">
            <p className="text-[10px] text-muted-foreground mb-1">Người chơi</p>
            <p className="font-bold text-lg text-foreground">{stats.total_players}</p>
          </div>
          <div className="rounded-xl bg-secondary/50 p-2 text-center border border-border/50">
            <p className="text-[10px] text-muted-foreground mb-1">Điểm TB</p>
            <p className="font-bold text-lg text-foreground">{Math.round(stats.average_score)}</p>
          </div>
          <div className="rounded-xl bg-secondary/50 p-2 text-center border border-border/50">
            <p className="text-[10px] text-muted-foreground mb-1">Đúng TB</p>
            <p className="font-bold text-lg text-primary">{Math.round(stats.average_correct_pct)}%</p>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
             <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
               Tỉ lệ đúng theo câu hỏi
             </p>
             <div className="flex gap-2 text-[9px] text-muted-foreground">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#1D9E75]"></span>Dễ</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#BA7517]"></span>TB</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#E24B4A]"></span>Khó</span>
             </div>
          </div>

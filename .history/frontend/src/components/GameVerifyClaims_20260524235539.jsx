...
export const GameVerifyClaims = () => {
  ...
  // --- THÊM MỚI: Gọi API khi mount component ---
  useEffect(() => {
  ...
  // ... (giữ nguyên các hàm handleStart, handleChoose, handleNext, handleReplay)

  // --- THÊM MỚI: Component hiển thị thống kê nhỏ ---
  const StatsSummary = () => {
    if (loadingStats) return <p className="text-xs text-muted-foreground">Đang tải thống kê...</p>;
    if (!stats || stats.total_players === 0) return <p className="text-xs text-muted-foreground">Chưa có người chơi nào.</p>;
    ...
    return (
      <div className="mt-4 rounded-xl bg-secondary/50 p-3 text-sm">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
          <div>
            <span className="block font-bold text-foreground">{stats.total_players}</span>
            Người chơi
          </div>
          <div>
            <span className="block font-bold text-foreground">{Math.round(stats.average_correct_pct)}%</span>
        ...
        {stats.claim_stats && stats.claim_stats.length > 0 && (
           <div className="mt-3 pt-3 border-t border-border">
             ...
             <div className="flex flex-wrap gap-1">
               {stats.claim_stats.slice(0, 3).map((c, i) => (
                 <span key={i} className="px-2 py-1 rounded-md bg-background border border-border text-[10px]">
                   Câu {i+1}: {Math.round(c.correct_pct)}% đúng
                 </span>
               ))}
             </div>
           </div>

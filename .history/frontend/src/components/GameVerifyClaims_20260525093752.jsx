...
export const GameVerifyClaims = () => {
  ...
  const handleReplay = () => {
    ...
    if (loadingStats) {
      return (
        <div className="mt-4 rounded-xl bg-secondary/50 p-3 text-sm animate-pulse">
          <p className="text-xs text-muted-foreground">Đang cập nhật số liệu từ server...</p>
        </div>
      );
    }

    // 2. Hiển thị khi không lấy được dữ liệu (Lỗi mạng/API)
    if (!stats) {
      return (
        <div className="mt-4 rounded-xl bg-red-50 border border-red-100 p-3 text-sm">
          <p className="text-xs text-red-600 font-bold">⚠️ Không nhận được dữ liệu.</p>
          <p className="text-[10px] text-red-500 mt-1">Kiểm tra Console (F12) để xem lỗi Network.</p>
        </div>
      );
    }

    // 3. Hiển thị dữ liệu thực tế (Kể cả khi total_players = 0)
    return (
      <div className="mt-4 rounded-xl bg-secondary/50 p-3 text-sm border border-border/50">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
          <div>
            <span className="block font-bold text-foreground text-lg">{stats.total_players}</span>
            Người chơi
          </div>
          <div>
            <span className="block font-bold text-foreground text-lg">
              {stats.average_correct_pct ? Math.round(stats.average_correct_pct) : 0}%
            </span>
            Tỉ lệ đúng TB
          </div>
        </div>
        
        {/* Hiển thị chi tiết từng câu hỏi nếu có */}
           ...
           <div className="mt-3 pt-3 border-t border-border/50">
             ...
             <div className="space-y-2">
               {stats.claim_stats.slice(0, 3).map((c, i) => {
                 const saiPct = 100 - (c.correct_pct || 0);
                 return (
                   <div key={i} className="flex items-center justify-between text-[11px]">
                     <span>Câu {i + 1}</span>
                     <span className={`font-bold ${saiPct > 50 ? 'text-red-500' : 'text-green-600'}`}>
                       {Math.round(saiPct)}% bị sai
                     </span>
                   </div>
                 );
               })}
